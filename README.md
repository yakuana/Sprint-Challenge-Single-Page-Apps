...
> Answer: We can use JavaScript to maintain State or memory within our applications and use that memory to tell the Browser what to display when a resource is requested.
...
> Answer: Controlled components allow an application to have the power to dictate what kinds of values our inputs are permitted to have. The value of the inputs are explicitly defined by state.
